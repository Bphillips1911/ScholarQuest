...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  const authenticateTeacher = async (req: any, res: any, next: any) => {
    ...
    try {
      ...
      // Always require database validation for security
      const teacher = await storage.getTeacherAuthById(decoded.teacherId);
      
      if (!teacher) {
        console.log(`AUTH ERROR: Teacher not found in database: ${decoded.teacherId}`);
        return res.status(401).json({ message: "Invalid token" });
      }
  ...
  // Get teacher's scholars for admin viewing
  app.get("/api/teacher/scholars/:teacherId", authenticateAdmin, async (req, res) => {
    try {
      ...
      const scholars = await storage.getScholarsByGrade(teacher.gradeRole);
      res.json(scholars);
    } catch (error) {
      console.error("Error fetching teacher scholars:", error);
      res.status(500).json({ message: "Failed to fetch scholars" });
    }
  });

  // Get teacher's reflections for admin viewing
  app.get("/api/teacher/reflections/:teacherId", authenticateAdmin, async (req, res) => {
    try {
      const { teacherId } = req.params;
      const reflections = await storage.getReflectionsByTeacher(teacherId);
  ...
  // Get teacher's messages for admin viewing
  app.get("/api/teacher/messages/:teacherId", authenticateAdmin, async (req, res) => {
    try {
      const { teacherId } = req.params;
  ...
  // Get teacher's photos for admin viewing
  app.get("/api/teacher/photos/:teacherId", authenticateAdmin, async (req, res) => {
    try {
      const { teacherId } = req.params;
      const photos = await storage.getPhotosByTeacher(teacherId);
      res.json(photos);
    } catch (error) {
  ...
  const { progressReportService } = await import('./progressReportService');
  const { teacherPerformanceService } = await import('./teacherPerformanceService');
  ...
  app.put("/api/teacher/recommendations/:id/status", authenticateTeacher, async (req, res) => {
    try {
      const { id } = req.params;
      const { status, effectivenessRating } = req.body;
      
      const token = req.headers.authorization?.replace("Bearer ", "");
      const session = await storage.getTeacherSession(token);
      const teacherId = session?.teacherId;

      await aiRecommendationService.updateRecommendationStatus(id, status, teacherId, effectivenessRating);
      res.json({ success: true });
    } catch (error) {
  ...
  app.get("/api/teacher/recommendations/class/:grade", authenticateTeacher, async (req, res) => {
    try {
      ...
      const classRecommendations = await aiRecommendationService.generateClassRecommendations(teacherId, parseInt(grade));
      res.json(classRecommendations);
    } catch (error) {
      console.error("Get class recommendations error:", error);
      res.status(500).json({ message: "Failed to get class recommendations" });
    }
  });
  ...
  app.get("/api/teacher/progress-reports/:studentId", authenticateTeacherOrAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to get student reports" });
    }
  });

  app.get("/api/teacher/my-reports", authenticateTeacher, async (req, res) => {
    try {
      const token = req.headers.authorization?.replace("Bearer ", "");
      const session = await storage.getTeacherSession(token);
      const teacherId = session?.teacherId;

      const reports = await progressReportService.getTeacherReports(teacherId);
      res.json(reports);
    } catch (error) {
      console.error("Get teacher reports error:", error);
      res.status(500).json({ message: "Failed to get teacher reports" });
    }
  });
  ...
  app.get("/api/admin/performance-heatmap", async (req, res) => {
    try {
      ...
      if (!admin || (admin.title !== 'Principal' && admin.title !== 'Assistant Principal')) {
        return res.status(403).json({ message: "Access restricted to Principal and Assistant Principal only" });
      }
      ...
      res.status(500).json({ message: "Failed to get performance heatmap data" });
    }
  });
  ...
  app.post("/api/admin/calculate-teacher-metrics", async (req, res) => {
    try {
      ...
      await teacherPerformanceService.calculateAllTeacherMetrics(targetDate);
      res.json({ success: true, message: "Teacher metrics calculated successfully" });
    } catch (error) {
      console.error("Calculate teacher metrics error:", error);
      res.status(500).json({ message: "Failed to calculate teacher metrics" });
    }
  });

  app.get("/api/admin/top-performers/:metric", authenticateTeacher, async (req, res) => {
    try {
      const token = req.headers.authorization?.replace("Bearer ", "");
      const adminSession = await storage.getAdminSession(token);
      ...
      const topPerformers = await teacherPerformanceService.getTopPerformers(
        metric as 'effectiveness' | 'engagement' | 'communication',
  ...
  // Achievement Playground Routes
  app.get("/api/student/playground/:studentId", async (req, res) => {
    try {
      const { studentId } = req.params;
      ...
      const token = req.headers.authorization?.replace("Bearer ", "");
      if (token) {
        const studentSession = await storage.getStudentSession(token);
        if (!studentSession || studentSession.studentId !== studentId) {
          return res.status(403).json({ message: "Access denied" });
        }
      }

      const playground = await achievementPlaygroundService.getStudentPlayground(studentId);
      res.json(playground);
    } catch (error) {
      console.error("Get student playground error:", error);
      res.status(500).json({ message: "Failed to get student playground" });
    }
  });

  app.post("/api/student/playground/init/:studentId", async (req, res) => {
    try {
      const { studentId } = req.params;
      await achievementPlaygroundService.initializeStudentPlayground(studentId);
      res.json({ success: true, message: "Achievement playground initialized" });
    } catch (error) {
      console.error("Initialize playground error:", error);
      res.status(500).json({ message: "Failed to initialize achievement playground" });
    }
  });

  app.post("/api/student/playground/update/:studentId", async (req, res) => {
    try {
      const { studentId } = req.params;
      const newAchievements = await achievementPlaygroundService.updateStudentProgress(studentId);
      res.json({ 
        success: true, 
        newAchievements,
        message: `Updated progress and unlocked ${newAchievements.length} new achievements` 
      });
    } catch (error) {
      console.error("Update playground progress error:", error);
      res.status(500).json({ message: "Failed to update playground progress" });
    }
  });

  app.post("/api/student/playground/celebration/:achievementId", async (req, res) => {
    try {
      const { achievementId } = req.params;
      await achievementPlaygroundService.markCelebrationShown(achievementId);
      res.json({ success: true });
    } catch (error) {
      console.error("Mark celebration shown error:", error);
      res.status(500).json({ message: "Failed to mark celebration shown" });
    }
  });

  app.post("/api/student/playground/share/:achievementId", async (req, res) => {
    try {
      const { achievementId } = req.params;
      await achievementPlaygroundService.shareAchievement(achievementId);
      res.json({ success: true, message: "Achievement shared successfully" });
    } catch (error) {
  ...
  app.get("/api/student/playground/leaderboard/:achievementId", async (req, res) => {
    try {
      const { achievementId } = req.params;
      const { limit } = req.query;
      
      const leaderboard = await achievementPlaygroundService.getAchievementLeaderboard(
        achievementId,
  ...
  // Background task to update all student progress (can be called periodically)
  app.post("/api/admin/update-all-progress", authenticateTeacher, async (req, res) => {
    try {
      const token = req.headers.authorization?.replace("Bearer ", "");
      const adminSession = await storage.getAdminSession(token);
      ...
      const scholars = await storage.getAllScholars();
      const activeScholars = scholars.filter(s => s.isActive);
      
      let updatedCount = 0;
      for (const scholar of activeScholars) {
        try {
          await achievementPlaygroundService.updateStudentProgress(scholar.id);
          updatedCount++;
        } catch (error) {
          console.error(`Error updating progress for student ${scholar.id}:`, error);
        }
      }

      res.json({ 
        success: true, 
        message: `Updated progress for ${updatedCount} students`,
        totalStudents: activeScholars.length
      });
    } catch (error) {
      console.error("Update all progress error:", error);
      res.status(500).json({ message: "Failed to update all student progress" });
    }
  });
